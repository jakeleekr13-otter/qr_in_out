...
```
[본사 네트워크]─────────[VPN 서버]─────────[원격 사무실 네트워크]
      │                                            │
[Streamlit 서버]                          [Host 기기 + Guest]
192.168.1.100:8501                         (VPN 클라이언트)
```

- **사용 사례**: 기업 네트워크, 고보안 환경
- **설정**: VPN 터널을 통해 원격 사이트 연결
- **Guest 접근**: VPN 연결 후 `http://192.168.1.100:8501` (내부 IP)

**참고**: 이 시나리오에서는 Guest도 VPN 클라이언트가 필요합니다. 외부 방문객(VPN 없음)을 위해서는 시나리오 2(클라우드 배포)를 권장합니다.
...
| 53 | UDP/TCP | 아웃바운드 | DNS (도메인 이름 확인) |
...
- [x] **네트워크 아키텍처 문서**: 배포 가이드 및 방화벽 설정 (v1.2 완료)
- [x] **WiFi 정보 표시**: Host 화면에 네트워크 자격 증명 표시 (v1.2 완료)
- [x] **연결 상태 모니터링**: 실시간 서버 및 API 상태 (v1.2 완료)
- [x] **사운드 피드백**: 스캔 결과에 대한 오디오 확인 (v1.2 완료)
- [x] **정보 기억하기**: 방문객 자격 증명 로컬 저장 (v1.2 완료)
- [x] **키오스크 모드**: 공유 기기를 위한 연속 스캔 (v1.2 완료)
- [x] **고대비 QR**: 접근성을 위한 여러 표시 모드 (v1.2 완료)
- [ ] **다국어 지원**: 국제화 (i18n)
